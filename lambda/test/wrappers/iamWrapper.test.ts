...
describe("IAMWrapper", () => {
    ...
    const createMockGetRoleResponse = (overrides: Partial<GetRoleCommandOutput> = {}): GetRoleCommandOutput => ({
        Role: {
    ...
    const createMockGetProfileResponse = (overrides: Partial<GetInstanceProfileCommandOutput> = {}): GetInstanceProfileCommandOutput => ({
        InstanceProfile: {
    ...
    describe("getRole", () => {
        ...
        it("should throw error if role has no name", async () => {
            iamMock.on(GetRoleCommand).resolves(
                createMockGetRoleResponse({ Role: { RoleName: undefined } as any })
            );

            await expect(iamWrapper.getRole()).rejects.toThrow('Failed to create Role - no name returned');
        });
    ...
    describe("createRole", () => {
        it("should successfully create role with SSM policy attached", async () => {
            ...
            const trustPolicy = JSON.parse(createRoleCall.args[0].input.AssumeRolePolicyDocument || '{}');
    ...
    describe("attachSSMPolicy", () => {
        ...
        it("should handle policy already attached gracefully", async () => {
            ...
            await expect(iamWrapper.attachSSMPolicy(mockRoleName, mockPolicyArn)).resolves.toBeUndefined();
        });
        ...
        it("should throw error for other failures", async () => {
            ...
            await expect(iamWrapper.attachSSMPolicy(mockRoleName, mockPolicyArn))
                .rejects.toThrow('Invalid policy ARN');
        });
    });
    ...
    describe("getProfile", () => {
        ...
        it("should throw error if profile has no ARN", async () => {
            ...
            await expect(iamWrapper.getProfile()).rejects.toThrow('Failed to create instance profile - no name returned');
        });
    ...
    describe("attachRoletoProfile", () => {
        ...
        it("should handle role already attached gracefully", async () => {
            ...
            await expect(iamWrapper.attachRoletoProfile(mockProfileName, mockRoleName))
