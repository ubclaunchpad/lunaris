...
describe("deploymentStatus handler", () => {
    ...
    describe("DynamoDB queries", () => {
        it.each([
            ...
            },
            {
                ...
                dynamoResponse: { Items: [{ instanceId: "i-123456", userId: "test-user", status: "running" }], Count: 1 },
            ...
            const result = await handler(createMockEvent("test-user") as APIGatewayProxyEvent, mockContext, mockCallback);
    ...
    describe("Step Functions execution statuses", () => {
        ...
        const baseResponse = { executionArn: arn, stateMachineArn: "arn:aws:states:us-east-1:123456789012:stateMachine:DeployEC2", startDate: new Date() };

        beforeEach(() => {
            dynamoDBMock.on(QueryCommand).resolves({ Items: [{ instanceId: "i-123", userId: "test-user", executionArn: arn }], Count: 1 });
        });

        it.each([
            { status: "RUNNING" as const, expects: { status: "RUNNING", deploymentStatus: "deploying", message: "Deployment in progress..." } },
            { status: "SUCCEEDED" as const, output: JSON.stringify({ instanceId: "i-123", dcvUrl: "https://54.123.45.67:8443" }), expects: { status: "SUCCEEDED", deploymentStatus: "running", message: "Instance is ready for streaming", instanceId: "i-123", dcvUrl: "https://54.123.45.67:8443" } },
            { status: "SUCCEEDED" as const, expects: { status: "SUCCEEDED", instanceId: "i-123" } },
            { status: "FAILED" as const, error: "InstanceLimitExceeded", cause: "account limit reached", expects: { status: "FAILED", error: "InstanceLimitExceeded", messageContains: "account limit reached" } },
            ...
            sfnMock.on(DescribeExecutionCommand).resolves({ ...baseResponse, status, ...(output && { output, stopDate: new Date() }), ...(error && { error, cause, stopDate: new Date() }), ...(!output && !error && status !== "RUNNING" && { stopDate: new Date() }) } as any);
            ...
            if (expects.deploymentStatus) expect(body.deploymentStatus).toBe(expects.deploymentStatus);
        ...
        it.each([
            {
                ...
                setupMock: () => dynamoDBMock.on(QueryCommand).rejects(new Error("DynamoDB connection failed")),
            ...
            },
            {
                ...
                setupMock: () => {
                    dynamoDBMock.on(QueryCommand).resolves({
                        Items: [{ instanceId: "i-123", userId: "test-user", executionArn: "arn:aws:states:us-east-1:123456789012:execution:test:exec" }],
                        Count: 1,
                    });
                    sfnMock.on(DescribeExecutionCommand).rejects(new Error("ExecutionDoesNotExist"));
                },
    ...
    it("should never expose executionArn in response", async () => {
        ...
        dynamoDBMock.on(QueryCommand).resolves({ Items: [{ instanceId: "i-123", userId: "test-user", executionArn: arn }], Count: 1 });
        sfnMock.on(DescribeExecutionCommand).resolves({ status: "RUNNING" as const, executionArn: arn, stateMachineArn: "arn:aws:states:us-east-1:123456789012:stateMachine:test", startDate: new Date() });
