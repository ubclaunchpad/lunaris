...
describe("user-terminate-ec2/update-running-instances", () => {
    ...
    it("sets status from the event in ExpressionAttributeValues", async () => {
        await handler(BASE_EVENT);
    ...
    it("accepts any status value passed in the event (e.g. 'terminated')", async () => {
        await handler({ ...BASE_EVENT, status: "terminated" });

        const updateConfig = (mockDynamoDBWrapper.updateItem as jest.Mock).mock
            .calls[0][0] as Record<string, unknown>;
        const eav = updateConfig.ExpressionAttributeValues as Record<string, string>;
        expect(eav[":status"]).toBe("terminated");
    });
