...
function generateWindowsUserData(dcvPassword: string): string {
    ...
    // The AMI is expected to have NICE DCV pre-installed; this script ensures
    // the service/session are ready and installs a browser-trusted certificate
    // for the current public nip.io hostname so the frontend can use WSS.
    return `<powershell>
# Log startup
...
# Ensure Windows firewall allows inbound DCV HTTPS traffic
try {
    $dcvRule = Get-NetFirewallRule -DisplayName "Lunaris DCV HTTPS" -ErrorAction SilentlyContinue
    if (-not $dcvRule) {
...
try {
    $acmeRule = Get-NetFirewallRule -DisplayName "Lunaris ACME HTTP" -ErrorAction SilentlyContinue
    if (-not $acmeRule) {
...
# Install or refresh a browser-trusted certificate for the current public nip.io hostname
try {
    ...
    $CertFile = Get-ChildItem -Path "C:\\DCV-Certs" -Filter "*-chain.pem" | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    if (-not $CertFile) {
        $CertFile = Get-ChildItem -Path "C:\\DCV-Certs" -Filter "*-fullchain.pem" | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    }
    if (-not $CertFile) {
    ...
    if ($CertFile -and $KeyFile) {
        ...
        Start-Sleep -Seconds 10
        $dcvPortTest = Test-NetConnection -ComputerName localhost -Port 8443 -WarningAction SilentlyContinue
        if ($dcvPortTest.TcpTestSucceeded) {
