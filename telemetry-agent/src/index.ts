...
async function getInstanceId(): Promise<string> {
  ...
  try {
    ...
    if (!res.ok) throw new Error(`IMDS returned ${res.status}`);
...
function getUserId(): string {
  ...
  if (!v) throw new Error("USER_ID env var is required");
  return v;
}

function getSessionId(): string {
  const v = process.env.SESSION_ID;
  if (!v) throw new Error("SESSION_ID env var is required");
  return v;
}
 ...
 */
async function checkDcvSession(): Promise<boolean> {
